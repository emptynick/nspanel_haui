...
from .haui.device_config import LOCALE_OPTIONS
...
class ConfigSchema:
    ...
    def user_panel_types() -> list[SelectOptionDict]:
        ...
        try:
            ...
            _LOGGER.warning("Could not load panel types", exc_info=True)
            raise
    ...
    def panel_label(panel: dict) -> str:
        ...
        if title:
            return title
        key = panel.get("key") or ""
        return key or panel.get("type", "?")
    ...
    def panel_edit_schema(current: dict, panel_types: list[SelectOptionDict]) -> vol.Schema:
        ...
        return vol.Schema(
            {
                vol.Required(
                    "type", default=current.get("type", default_type)
    ...
    @staticmethod
    def panel_type_specific_schema(
        panel_type: str, current: dict | None = None
    ) -> tuple[dict, dict]:
        """Build a schema dict and transform map from PageOptions on the panel's page class.
        ...
        for opt in ConfigSchema.panel_type_options(panel_type):
            ...
            if opt.kind == "bool":
                ...
                # Accept RGB565 int, "[r,g,b]" string, or "#rrggbb" / template string.
                if isinstance(cur, int):
                    default = cur
                else:
                    default = str(cur) if cur is not None else ""
                schema[vol.Optional(opt.key, default=default)] = vol.Any(int, str)
                # No drop_empty — int 0 is a valid RGB565 color (black)
    ...
    def apply_transforms(user_input: dict, transforms: dict) -> dict:
        ...
        Transforms are declared alongside the schema by
    ...
    def build_panel_schema(
        current: dict, panel_types: list[SelectOptionDict], user_input: dict | None = None
    ) -> vol.Schema:
